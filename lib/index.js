...
function writeJson(res, status, value) {
  ...
  try {
    ...
    // Client connection already gone (e.g. the tab was closed while the LLM was
    // still generating). The business work is already committed on the Host —
    // the report id stays in `outputs` and the docx is on disk — so a dead
    // response must never surface as an unhandled rejection.
  }
}
