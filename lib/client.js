...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const CSS = [
			...
			".drp-close{border:0;background:#e5e7eb;color:#111827;font-size:26px;line-height:1;cursor:pointer;border-radius:8px;width:34px;height:34px}",
			...
			"@media(max-width:760px){.drp-boxes{grid-template-columns:1fr}.drp-textarea{min-height:180px}.drp-overlay{padding:8px}.drp-panel{padding:16px}}"
		].join("");
		...
		function Overlay() {
			...
			react.useEffect(function () {
				const listener = function (value) { setOpen(value); };
			...
			const pickFiles = async function (period, event) {
				...
				const file = files[0];
				if (file.size > MAX_UPLOAD_BYTES) { setError("文件超过 100MB，请先压缩（源代码文件夹请打包为 .zip）"); return; }
				setUploading(period);
				setError("");
			...
			// Native change listeners: React's synthetic events can miss file
			// inputs in some environments, so bind the real DOM event too.
			react.useEffect(function () {
				if (!open) return;
				const handlers = {};
				["morning", "afternoon"].forEach(function (period) {
					...
					const handler = function (event) { pickFiles(period, event); };
			...
			const material = function (title, key, period) {
				return react.createElement("label", { className: "drp-box" },
					...
					react.createElement("div", { className: "drp-upload" },
						...
						react.createElement("input", { type: "file", className: "drp-file-input", ref: function (node) { fileInputs.current[period] = node; }, accept: ".zip,.docx,.txt,.md,.json,.js,.ts,.tsx,.jsx,.py,.java,.c,.h,.cpp,.cs,.go,.rs,.sql,.html,.css,.scss,.xml,.yaml,.yml,.toml,.ini,.sh,.ps1,.vue,.php,.rb,.swift,.kt,.properties", onChange: function (event) { pickFiles(period, event); } }),
			...
			return react.createElement("div", { className: "drp-overlay" },
				react.createElement("div", { className: "drp-mask", onClick: function () { if (!busy && !sending && !uploading) window.__drpSetOpen(false); } }),
				react.createElement("section", { className: "drp-panel", role: "dialog", "aria-modal": "true", "aria-label": "日报助手" },
					react.createElement("header", { className: "drp-head" },
						...
						react.createElement("button", { type: "button", className: "drp-close", "aria-label": "关闭", disabled: !!busy || !!sending || !!uploading, onClick: function () { window.__drpSetOpen(false); } }, "×")),
