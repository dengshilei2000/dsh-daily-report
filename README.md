...
把整个插件目录放到本机任意无空格路径（例如 `D:\daily-report-plugin`），然后：

```bash
dsh plugin --profile web add link:D:/daily-report-plugin
```

### 方式二：tarball 安装（分发给他人）

```bash
# 在插件目录内打包
cd D:\daily-report-plugin
pnpm pack   # 或 npm pack，生成 dsh-daily-report-0.1.0.tgz
