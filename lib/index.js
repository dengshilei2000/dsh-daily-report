...
// Exposes HTTP routes under /api/daily-report/* for the browser half.
...
function normalizeInput(raw) {
  ...
  if (!morningMaterials && !afternoonMaterials) throw new Error('请至少填写上午素材或下午素材')
...
async function createReport(ctx, data) {
  ...
  const options = {
    ...
    messages: [{ id: 'daily-report-' + String(++sequence), role: 'user', content: [{ type: 'text', text: promptText }], source: { kind: 'plugin', plugin: 'daily-report-assistant' } }],
...
async function createReportWithFiles(ctx, data, morningUploads, afternoonUploads) {
  const sections = []
  if (data.morningMaterials) sections.push('【上午素材】\n' + data.morningMaterials)
  if (data.afternoonMaterials) sections.push('【下午素材】\n' + data.afternoonMaterials)
...
async function saveDocx(ctx, report) {
...
async function sendDingTalk(ctx, recipient, file) {
